...
func NewSocialAuthService(cfg *config.Config, redisService *RedisService) *SocialAuthService {
...
func (s *SocialAuthService) CompleteAuth(ctx context.Context, provider, code, state string) (*SocialIdentity, error) {
...
func (s *SocialAuthService) completeGoogleAuth(ctx context.Context, code, nonce string) (*SocialIdentity, error) {
...
func (s *SocialAuthService) completeAppleAuth(ctx context.Context, code, nonce string) (*SocialIdentity, error) {
	...
	if identity.ProviderUserID == "" {
		return nil, errors.New("apple account did not return a subject")
	}
	return identity, nil
}
...
func (s *SocialAuthService) buildAppleClientSecret() (string, error) {
	...
	secret, err := jwtToken.SignedString(key)
	if err != nil {
		return "", err
	}
	return secret, nil
...
func (s *SocialAuthService) verifyIDToken(idToken, jwksURL string, audience []string, issuers []string, nonce string) (jwt.MapClaims, error) {
	...
	if !token.Valid {
		return nil, errors.New("invalid social id token")
	}
...
func (s *SocialAuthService) fetchJWKS(jwksURL string) (map[string]*rsa.PublicKey, error) {
	...
	for _, item := range body.Keys {
		...
		pub, err := jwkToRSAPublicKey(item)
		if err != nil {
			continue
		}
		keys[item.Kid] = pub
	...
	if len(keys) == 0 {
		return nil, errors.New("no jwks keys loaded")
	}

	s.cacheMu.Lock()
	s.jwksCache[jwksURL] = jwksCacheEntry{keys: keys, expiresAt: time.Now().Add(jwksCacheTTL)}
	s.cacheMu.Unlock()
	return keys, nil
}

func jwkToRSAPublicKey(item jwk) (*rsa.PublicKey, error) {
